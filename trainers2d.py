"""Implementations of 2D trainers and evaluators
"""
...
from models.builder import build_model_2d
...
class DiceTrainer:
    ...
    def __init__(self, config: dict):
        ...
        self.network = build_model_2d(model_config).to(self.device)
        ...
        for epoch in iters:
            ...
            for sample in self.val_loader:
                ...
                with torch.no_grad():
                    ...
                    avg_hd += self.hd95(fixed_seg, moving_seg_warped, spacing=(1.56, 1.56))
...
class DiceTester:
    ...
    def run(self):
        ...
        for batch_id, sample in enumerate(tqdm(self.loader)):            
            ...
            forward_hd95 = self.hd95(fixed_seg, moving_seg_warped, spacing=(1.56, 1.56))
            backward_hd95 = self.hd95(moving_seg, fixed_seg_warped, spacing=(1.56, 1.56))
            forward_assd = self.assd(fixed_seg, moving_seg_warped, spacing=(1.56, 1.56))
            backward_assd = self.assd(moving_seg, fixed_seg_warped, spacing=(1.56, 1.56))
...
class TRETester:
    ...
    def run(self):
        ...
        performance_summary = [
            ...
            ['Avg. NCC', f'{mean_f_ncc:.4f} +- {std_f_ncc:.4f}', f'{mean_b_ncc:.4f} +- {std_b_ncc:.4f}'],
            ['Folding %', f'{mean_f_jac_det:.4f} +- {std_f_jac_det:.4f}', f'{mean_b_jac_det:.4f} +- {std_b_jac_det:.4f}'],
            ['Structured Dice', f'{mean_f_tre:.4f} +- {std_f_tre:.4f}', f'{mean_b_tre:.4f} +- {std_b_tre:.4f}'],
            ['SSIM', f'{mean_f_ssim:.4f} +- {std_f_ssim:.4f}', f'{mean_b_ssim:.4f} +- {std_b_ssim:.4f}'],
        ]
